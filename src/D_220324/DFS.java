...
public class DFS 
{
	static void DFS(int[][] graph, int start_node)//시작할 node를 받아와서 BFS탐색
	{
		ArrayList<Integer> visit = new ArrayList<>();//방문한 노드를 순서대로 저장하는 visit 배열
		Stack<Integer> stack = new Stack<>();//방문할 노드를 저장하는 queue
		
		stack.push(start_node);
		
		while(!stack.isEmpty())//queue안에 후보노드가 존재한다면 탐색을 계속 진행
		{
			int node = stack.pop();//방문할 후보노드를 queue에서 뽑음
